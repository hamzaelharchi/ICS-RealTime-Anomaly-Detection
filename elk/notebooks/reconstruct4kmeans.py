...
scaler = joblib.load('work/ocsvm/transformers/scaler.pickle')
...
#loading the k means
k_means_model= joblib.load('work/kmeans.pickle')
...
def process_and_send_to_kafka(batch_df, batch_id):
    ...
    if not batch_df.isEmpty():  # Check if the batch is not empty
        ...
        #for _, row, row_raw in zip(preprocessed_data.iterrows() , preprocessed_data.iterrows()):
        for (_, row), (_, row_raw) in zip(preprocessed_data.iterrows(), pandas_df.iterrows()):
                ...
                try:
                    ...
                    for original_row, reconstructed_row in zip(preprocessed_data.iterrows(), reconstructed_df.iterrows()):
                        ...
                        if combined_dict["anomalyFlagged"] == 1: 
                            # Ensure original_dict contains numeric, non-NaN values only
                            numeric_values = [v for v in original_dict.values() if isinstance(v, (int, float)) and not pd.isna(v)]
                            
                            # Predict the cluster using the KMeans model
                            try:
                                cluster = k_means_model.predict([numeric_values])[0]  # Reshape into 2D array and get cluster
                                
                                # Convert row_raw (Series) into a single-row DataFrame
                                tosend = pd.DataFrame([row_raw], columns=pandas_df.columns)
                                
                                # Add the predicted cluster to the DataFrame
                                tosend['cluster'] = cluster
                                
                                # Convert the DataFrame to a dictionary for Kafka
                                tosend_dict = tosend.to_dict(orient='records')[0]  # Convert single-row DataFrame to dict
                                ...
                                producer.send(
                                    ...
                                    value=json.dumps(tosend_dict)  # Serialize as JSON
                                ...
                                print(f"Error in KMeans prediction or Kafka send: {e}")
                except Exception as e:
