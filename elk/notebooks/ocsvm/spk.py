...
if OUTPUT_TOPIC_NAME not in existing_topics:
...
kscaler = joblib.load('work/ocsvm/transformers/kscaler.pickle')
...
svm =  joblib.load('work/ocsvm/models/svm.pickle')
k_means_model= joblib.load('work/ocsvm/models/ocsvm_kmeans.pickle')
...
def process_and_send_to_kafka(batch_df, batch_id):
    if not batch_df.isEmpty():  # Check if the batch is not empty
        ...
        d = anomaly_data.drop(['Timestamp','Normal/Attack','prediction','index'],axis=1)
        d = kscaler.transform(d)
        cluster = k_means_model.predict(np.asarray(d)) 
        anomaly_data['clusters'] = cluster
        ...
        df_with_predictions = spark.createDataFrame(anomaly_data)
