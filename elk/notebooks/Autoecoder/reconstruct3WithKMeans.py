        ...
        for _, row in preprocessed_data.iterrows():
            ...
            if len(BUFFER) == BUFFER_SIZE:
                ...
                for original_row, reconstructed_row in zip(preprocessed_data.iterrows(), reconstructed_df.iterrows()):
                    ...
                    row_series = original_row[1]  # row[1] is the Pandas Series
                    row_array = row_series.to_numpy()  # Convert to NumPy array
                    ...
                    if combined_dict["anomalyFlagged"] == 1:
                        anomalies_data_pt.append(row_array)
                        print(f'this is an anomaly {row_array}')
                    producer.send(
                        OUTPUT_TOPIC_NAME, 
                    ...
                    # spdf= spark.createDataFrame(pandas_df)
                    # spdf.show()
                    # todict = pandas_df.to_dict()

                    # producer.send(
                    #     "kmeans_results", 
                    #     value = json.dumps(todict)
                    # )
            ...
            # Clear the buffer
            BUFFER = []
        if(len(anomalies_data_pt)!=0):
            clusters= k_means_model.predict([i.values.reshape(1, -1) for i in anomalies_data_pt]) 
            print("fjdksal;fjdksla;jfkdl;sajfklds;ajfklds;ajfierjfnkjdjfiodsnjfjdiofjegjofdghrkjgergn fgej jfkdhf") 
        else:
            print("ONSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS  NOKIAAAAAAAAAAAAAAAAAAAAA")
